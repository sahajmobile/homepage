...
// export default function Footer() {
//   // Generate particles once on client — no effect, no eslint error
//   const particles = useMemo(() => createParticles(), []);

//   // Safe year generation — also no effect needed
//   const year = useMemo(() => {
//     if (typeof window === 'undefined') return null; // SSR blank
...
//         {/* YEAR (hydration safe) */}
//         <div className="mt-12 border-t border-slate-800 pt-6 text-center text-xs text-slate-500">
//           © {year ?? ''} SahajMobile. All rights reserved.
//         </div>
//       </div>
...
//             animate={{ opacity: [0, 0.6, 0], y: [900, -50] }}
//             transition={{
//               duration: p.duration,
//               repeat: Infinity,
//               delay: p.delay,
...
export default function Footer() {
  const [year, setYear] = useState(''); // EMPTY on SSR → SAFE
  const [particles, setParticles] = useState([]); // EMPTY on SSR → SAFE
  ...
  useEffect(() => {
    ...
    const generate = Array.from({ length: 25 }).map(() => ({
      x: Math.random() * window.innerWidth,
      delay: Math.random() * 2,
      duration: 6 + Math.random() * 6,
      size: Math.random() * 2 + 1,
